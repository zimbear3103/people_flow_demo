...
public class UIHome : UIScreen
{
    ...
    {
        base.Show();
        m_maxLevel = AssetManager.Instance != null && AssetManager.Instance.GameLevels != null
            ? AssetManager.Instance.GameLevels.Datas.Count
            : PeopleFlow.PeopleFlowSampleLevels.Count;
        SetLevelUI();
    ...
    //private IEnumerator CapybaraMove(Vector3 pos)
    //{
    //    m_scrollLevel.enabled = false;
    //    while (Vector2.Distance(m_capybara.transform.position, pos) > 0.01f)
    //    {
    //        m_capybara.transform.position = Vector3.MoveTowards(m_capybara.transform.position, pos, m_speedCapybara * Time.deltaTime);
    //        yield return null;
    //    }
    //    m_capybara.transform.position = pos;
    //    m_scrollLevel.enabled = true;
    //}

    private void SetBackgroundByLevel(int level)
    {
        int index = level % m_listBackgrounds.Length;
        OnSetMainBackground(m_listBackgrounds[index]);
    }
